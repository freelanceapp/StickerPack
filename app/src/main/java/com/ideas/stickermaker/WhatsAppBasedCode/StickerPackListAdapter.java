...
import com.ideas.stickermaker.DataArchiver;
...
public class StickerPackListAdapter extends RecyclerView.Adapter<StickerPackListItemViewHolder> {
    @NonNull
    private List<StickerPackModal> stickerPacks;
    @NonNull
    private final OnAddButtonClickedListener onAddButtonClickedListener;
    private int maxNumberOfStickersInARow;
    private Context mContext;

    StickerPackListAdapter(Context mContext, @NonNull List<StickerPackModal> stickerPacks, @NonNull OnAddButtonClickedListener onAddButtonClickedListener) {
    ...
    public void onBindViewHolder(@NonNull final StickerPackListItemViewHolder viewHolder, final int index) {
        StickerPackModal pack = stickerPacks.get(index);
    ...
    private void setAddButtonAppearance(ImageView addButton, StickerPackModal pack) {
        if (pack.getIsWhitelisted()) {
            ...
            addButton.setImageResource(R.drawable.sticker_3rdparty_wa);
            addButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    AppPreference.setStringPreference(mContext, Constant.DownloadPack, "");
    ...
    public interface OnAddButtonClickedListener {
        void onAddButtonClicked(StickerPackModal stickerPack);
    }
}
