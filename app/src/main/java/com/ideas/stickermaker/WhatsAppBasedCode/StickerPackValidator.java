...
public class StickerPackValidator {
     ...
     */
    public static void verifyStickerPackValidity(@NonNull Context context, @NonNull StickerPackModal stickerPack) throws IllegalStateException {
        ...
        final List<StickerMainModal> stickers = stickerPack.getStickers();
        ...
        for (final StickerMainModal sticker : stickers) {
            validateSticker(context, stickerPack.identifier, sticker);
        }
    }

    private static void validateSticker(@NonNull Context context, @NonNull final String identifier, @NonNull final StickerMainModal sticker) throws IllegalStateException {
    ...
    private static Uri getStickerAssetUri(String identifier, String stickerName) {
        return new Uri.Builder().scheme(CustomStickerContentProvider.CONTENT_SCHEME).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY).appendPath(CustomStickerContentProvider.STICKERS_ASSET).appendPath("2").appendPath(stickerName).build();
    }
}
