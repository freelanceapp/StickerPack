...
public class RetrofitService {
    ...
    public static void getStickerPackData(final Dialog dialog, final Call<StickerSubcategory> method, final WebResponse webResponse) {
        if (dialog != null)
            AppProgressDialog.show(dialog);

        method.enqueue(new Callback<StickerSubcategory>() {
            @Override
            public void onResponse(Call<StickerSubcategory> call, Response<StickerSubcategory> response) {
                if (dialog != null)
                    AppProgressDialog.hide(dialog);
                WebServiceResponse.handleResponse(response, webResponse);
            }
            ...
            @Override
            public void onFailure(Call<StickerSubcategory> call, Throwable throwable) {
                if (dialog != null)
                    AppProgressDialog.hide(dialog);
                webResponse.onResponseFailed(throwable.getMessage());
            }
